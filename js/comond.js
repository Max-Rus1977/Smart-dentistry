...
cloneNavTeg.classList.add('_slick-nav');
const cloneNavTegLi = cloneNavTeg.querySelectorAll('li');

document.body.prepend(cloneNavTeg);

const section = document.querySelectorAll('._sect-js');
  ...
  if (scrollDistance > 450) {
    ...
    section.forEach((el, i) => {
      if (el.offsetTop - document.querySelector('nav').clientHeight <= scrollDistance) {
        cloneNavTegLi.forEach((el) => {
          if (el.classList.contains('_acktiv')) {
            el.classList.remove('_acktiv');
          }
        });

        cloneNavTegLi[i].classList.add('_acktiv');
      }
    });
  }
  else {
    cloneNavTeg.classList.remove('_slick');
  }
});
